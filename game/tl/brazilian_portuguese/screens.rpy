translate brazilian_portuguese strings:

    # game/screens.rpy:433
    old "~ Demoned Away Chapter ~\nWelcome to the world of Higurashi: When They {color=#ff0000}C{/color}ry.\nThe Onikakushi arc will be the opening, inviting you into this world.\nSit back, relax and enjoy life in Hinamizawa to the fullest.\n\nThe difficulty is extremely high,\nbut I hope you will enjoy the reward."
    new "~ Capítulo: Arrebatado pelo Demônio ~\nBem vindo ao mundo de Higurashi: When They {color=#ff0000}C{/color}ry.\nO arco onikakushi é o convite de abertura para este mundo.\nSente-se, relaxe e aproveite a vida em Hinamizawa ao máximo.\n\nA dificuldade é extremamente alta,\nmas eu espero que você aprecie a recompensa."

    # characters
    old "Keiichi's dad"
    new "Pai de Keiichi"
    old "Keiichi's mom"
    new "Mãe de Keiichi"
    old "Keiichi and others"
    new "Keiichi e outros"
    old "Gallery"
    new "Multidão"
    old "Nurse"
    new "Enfermeira"
    old "Principal"
    new "Diretor"
    old "Villager"
    new "Morador"
    old "Girl"
    new "Garota"
    old "Woman"
    new "Mulher"
    old "Detective"
    new "Detetive"
    old "Waitress"
    new "Garçonete"
    old "Delinquent"
    new "Delinquente"
    old "Forensic Investigator"
    new "Investigador Forense"
    old "Club Members"
    new "Membros do Clube"
    old "Policeman"
    new "Policial"
    old "Young boy"
    new "Garoto"
    old "Staff Member"
    new "Membro da Staff"
    old "Club Members and Old Man"
    new "Membros do Clube e Velho"
    old "Rena's dad"
    new "Pai de Rena"
    old "Sonozaki Representive"
    new "Representante Sonozaki"
    old "Festival Stall"
    new "Barraca de Festival"
    old "Middle-Aged Man"
    new "Homem de Meia-Idade"
    old "Okinomiya PS"
    new "DP de Okinomuiya"
    old "Reporter"
    new "Repórter"
    old "Doctor"
    new "Médico"

    # game/screens.rpy:875
    old "Use"
    new "Usar"

    # game/screens.rpy:876
    old "Don't use"
    new "Não usar"

    # game/screens.rpy:896
    old 'Video opening might contain minor spoilers.\nDo you want to enable it anyway?'
    new 'O vídeo de abertura pode conter pequenos spoilers.\nDeseja ativá-lo mesmo assim?'
    
    # game/screens.rpy:899
    old "Enable opening"
    new "Ativar abertura"

    # game/screens.rpy:900
    old "Disable opening"
    new "Desativar abertura"

    old "The Scenario Jump Mode is available."
    new "O Salto de Cenário está disponível."

    # game/screens.rpy:919
    old "onik_prologue"
    new "onik_prologue"

    # game/screens.rpy:920
    old "onik_001"
    new "onik_001"

    # game/screens.rpy:921
    old "onik_002"
    new "onik_002"

    # game/screens.rpy:922
    old "onik_003"
    new "onik_003"

    # game/screens.rpy:923
    old "onik_004"
    new "onik_004"

    # game/screens.rpy:924
    old "onik_005"
    new "onik_005"

    # game/screens.rpy:925
    old "onik_009"
    new "onik_009"

    # game/screens.rpy:926
    old "onik_009_02"
    new "onik_009_02"

    # game/screens.rpy:932
    old "onik_010"
    new "onik_010"

    # game/screens.rpy:933
    old "onik_011"
    new "onik_011"

    # game/screens.rpy:934
    old "onik_012"
    new "onik_012"

    # game/screens.rpy:935
    old "onik_013"
    new "onik_013"

    # game/screens.rpy:936
    old "onik_014"
    new "onik_014"

    # game/screens.rpy:937
    old "onik_014_02"
    new "onik_014_02"

    # game/screens.rpy:938
    old "onik_015"
    new "onik_015"

    # game/screens.rpy:939
    old "onik_015_03"
    new "onik_015_03"

    # game/screens.rpy:957
    old "wata_001"
    new "wata_001"

    # game/screens.rpy:958
    old "wata_002"
    new "wata_002"

    # game/screens.rpy:959
    old "wata_003"
    new "wata_003"

    # game/screens.rpy:960
    old "wata_004"
    new "wata_004"

    # game/screens.rpy:961
    old "wata_005"
    new "wata_005"

    # game/screens.rpy:962
    old "wata_006"
    new "wata_006"

    # game/screens.rpy:963
    old "wata_007"
    new "wata_007"

    # game/screens.rpy:964
    old "wata_008"
    new "wata_008"

    # game/screens.rpy:965
    old "wata_009"
    new "wata_009"

    # game/screens.rpy:966
    old "wata_009_02"
    new "wata_009_02"

    # game/screens.rpy:972
    old "wata_010"
    new "wata_010"

    # game/screens.rpy:973
    old "wata_010_02"
    new "wata_010_02"

    # game/screens.rpy:974
    old "wata_010_03"
    new "wata_010_03"

    # game/screens.rpy:975
    old "wata_010_04"
    new "wata_010_04"

    # game/screens.rpy:976
    old "wata_011"
    new "wata_011"

    # game/screens.rpy:977
    old "wata_011_02"
    new "wata_011_02"

    # game/screens.rpy:978
    old "wata_012"
    new "wata_012"

    # game/screens.rpy:979
    old "wata_012_02"
    new "wata_012_02"

    # game/screens.rpy:980
    old "wata_012_03"
    new "wata_012_03"

    # game/screens.rpy:981
    old "wata_ep_01"
    new "wata_ep_01"

    # game/screens.rpy:999
    old "tata_prologue"
    new "tata_prologue"

    # game/screens.rpy:1000
    old "tata_001"
    new "tata_001"

    # game/screens.rpy:1001
    old "tata_002"
    new "tata_002"

    # game/screens.rpy:1002
    old "tata_003"
    new "tata_003"

    # game/screens.rpy:1003
    old "tata_004"
    new "tata_004"

    # game/screens.rpy:1004
    old "tata_005"
    new "tata_005"

    # game/screens.rpy:1005
    old "tata_008"
    new "tata_008"

    # game/screens.rpy:1006
    old "tata_008_02"
    new "tata_008_02"

    # game/screens.rpy:1007
    old "tata_009"
    new "tata_009"

    # game/screens.rpy:1008
    old "tata_009_02"
    new "tata_009_02"

    # game/screens.rpy:1009
    old "tata_010"
    new "tata_010"

    # game/screens.rpy:1015
    old "tata_010_02"
    new "tata_010_02"

    # game/screens.rpy:1016
    old "tata_010_03"
    new "tata_010_03"

    # game/screens.rpy:1017
    old "tata_010_04"
    new "tata_010_04"

    # game/screens.rpy:1018
    old "tata_011"
    new "tata_011"

    # game/screens.rpy:1019
    old "tata_011_02"
    new "tata_011_02"

    # game/screens.rpy:1020
    old "tata_011_03"
    new "tata_011_03"

    # game/screens.rpy:1021
    old "tata_012"
    new "tata_012"

    # game/screens.rpy:1022
    old "tata_013"
    new "tata_013"

    # game/screens.rpy:1023
    old "tata_013_02"
    new "tata_013_02"

    # game/screens.rpy:1024
    old "tata_014"
    new "tata_014"

    # game/screens.rpy:1025
    old "tata_ep01"
    new "tata_ep01"

    # game/screens.rpy:1043
    old "hima_prologue"
    new "hima_prologue"

    # game/screens.rpy:1044
    old "hima_001"
    new "hima_001"

    # game/screens.rpy:1045
    old "hima_002"
    new "hima_002"

    # game/screens.rpy:1046
    old "hima_002_02"
    new "hima_002_02"

    # game/screens.rpy:1047
    old "hima_002_03"
    new "hima_002_03"

    # game/screens.rpy:1048
    old "hima_003"
    new "hima_003"

    # game/screens.rpy:1054
    old "hima_003_02"
    new "hima_003_02"

    # game/screens.rpy:1055
    old "hima_003_03"
    new "hima_003_03"

    # game/screens.rpy:1056
    old "hima_003_04"
    new "hima_003_04"

    # game/screens.rpy:1057
    old "hima_003_05"
    new "hima_003_05"

    # game/screens.rpy:1058
    old "hima_004"
    new "hima_004"

